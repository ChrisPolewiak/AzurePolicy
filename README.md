...
│   └── assignment-identities.json        # AUTO-GENERATED — in .gitignore, passed via artifact
├── source/
│   ├── EnterpriseALZ/
...
├── configuration/
│   ├── ALZPolicyAssignments.xlsx          # Source of truth — assignments (edit here)
...
│   └── README.md                         # Notes on configuration directory
├── pipelines/
...
│   └── cleanup.yml                       # Pipeline E: list or delete managed policy resources in Azure
├── scripts/
...
│   ├── update-initiatives.sh             # Deploys policyDefinitions and policySetDefinitions to Management Group
...
│   ├── generate_config_from_table.py     # Generates config/*.json from TSV/CSV files
...
Use when you are changing **which initiatives are active** (adding/removing initiatives from `docs/Azure-Policy.xlsx`) or updating definitions from a new snapshot.
...
Open `configuration/ALZPolicyAssignments.xlsx` and modify the relevant tabs (assignments and/or parameters).
...
> **Next step:** Run **Process C** (`update-initiatives`) and/or **Process D** (`update-assignments`).
...
| `definitionManagementGroupId` | `your-management-group-id` | Target Management Group for initiatives (and fallback for definitions without `metadata.targetManagementGroup`) |
| `targetInitiative` | `*` | Initiative name to deploy (`*` = all) |
| `targetDefinition` | `*` | Policy definition name to deploy (`*` = all) |
...
Open `configuration/ALZPolicyAssignments.xlsx`.
...
Open `configuration/ALZPolicyAssignments.xlsx` → **Parameters** tab and update parameter values for the relevant sets (Parameter Set).
...
1. Fetches the **`policy-generated`** artifact from Pipeline B
2. Runs `update-assignments.sh` — validation + `az deployment tenant what-if`
